window.Tetris = (function(){
  ...
  if (!tCanvas) return { start(){}, stop(){}, isRunning(){return false;}, move(){}, softDrop(){}, hardDrop(){}, rotate(){}, togglePause(){} };
  ...
  function drawNext(){ const m=next.m; nextCtx.clearRect(0,0,96,96); const offx=(96 - m[0].length*BS/2)/2; const offy=(96 - m.length*BS/2)/2; nextCtx.save(); nextCtx.translate(offx,offy); for(let y=0;y<m.length;y++) for(let x=0;x<m[y].length;x++) if(m[y][x]) drawHeartAt(nextCtx,x,y,next.c,BS/2); nextCtx.restore(); }
